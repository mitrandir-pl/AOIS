...
class AsocMemory:
    ...
    def __init__(self):
        for _ in range(self.__size):
            ...
            self.__list_to_add.append(DEFAULT_TABLE_RAW.copy())
    ...
    def add_element(self, word):
        if len(str(word)) == 16:
            ...
            self.__list_to_add[self.__list_to_add_counter] = list(word)
            for _ in range(self.__size):
                self.__table.append(DEFAULT_TABLE_RAW.copy())
            offset = 0
    ...
    def add(self, index1, index2):
        if len(self.__list_to_add[index1]) == len(self.__list_to_add[index2]):
            spis1 = self.__list_to_add[index1]
            spis2 = self.__list_to_add[index2]
            dop = 0
            rezult = []
            for i in range(len(spis1)-1,-1,-1):
                temp = int(spis1[i]) + int(spis2[i]) + dop
                if temp == 3:
                    dop = 1
                    rezult.append(1)
                elif temp == 2:
...
def main():
    try:
        table = AsocMemory()
        table.print_table()
        table.add_element("0000101001000111")
        table.print_table()
        table.add_element("0100101011010101")
        table.print_table()
        # table.ariphmetics()
        # table.print_table()
